const main = async () => {
  const [{ blockedSites }, { oneTimeUnblocked }, { temporarilyUnblocked }] =
    await Promise.all([
  ...
  if (oneTimeUnblocked.includes(currentHost)) {
    const updatedOneTimeUnblocked = oneTimeUnblocked.filter(
      (host) => host !== currentHost,
    );
    await chrome.storage.local.set({
      oneTimeUnblocked: updatedOneTimeUnblocked,
    });
    return;
  }

  const tempUnblockEntry = temporarilyUnblocked.find(
    (entry) => entry.host === currentHost,
  );
  if (tempUnblockEntry) {
    if (tempUnblockEntry.expiresAt > currentTime) {
      return;
    }
    const updatedTempUnblocked = temporarilyUnblocked.filter(
      (entry) => entry.host !== currentHost,
    );
    await chrome.storage.local.set({
      temporarilyUnblocked: updatedTempUnblocked,
    });
  }
